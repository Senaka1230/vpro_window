...
export const mouseDownHandler = (event, canvas,scale, createria, setCanvasState, setRestoreNum) => {
  if (event.target) {
    if(event.target.id.includes("Dim") && (!event.target.id.includes("totalDimH") && (!event.target.id.includes("totalDimV")))){
      inputToEditDimensions(canvas,scale, createria, setCanvasState, setRestoreNum, event.target);
    }
    else{
...
export const mouseMoveHandler = (event) => {
  if (event.target && event.target.id && event.target.isDragging) {
    if (event.target.id.includes("topframe")) {
      event.target.set({ left: initLeft });
      movingFlag = true;
    }
    else if (event.target.id.includes("leftframe")) { event.target.set({ top: initTop }); movingFlag = true; }
    else if (event.target.id.includes("rightframe")) { event.target.set({ top: initTop }); movingFlag = true; }
    else if (event.target.id.includes("bottomframe")) { event.target.set({ left: initLeft }); movingFlag = true; }
    else if (event.target.id.includes("verticalOneBar")) { event.target.set({ top: initTop }); movingFlag = true; }
    else if (event.target.id.includes("horizontalBar")) { event.target.set({ left: initLeft }); movingFlag = true; }
  }
};
...
export const mouseMoving = async (canvas, scale, createria, setRestoreNum, setCanvasState, setDeleteFlag) => {

  canvas.isDrawingMode = false;
  canvas.selection = true;

  var bb = (event) => mouseDownHandler(event, canvas,scale, createria, setCanvasState, setRestoreNum);
