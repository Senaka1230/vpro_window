...
export function inputToEditDimensions(canvas, scale, createria, setCanvasState, setRestoreNum, textObj) {
    ...
    if (event.key === 'Enter') {
      ...
      if (!isNaN(editedDim)) {
        ...
        if (textObj.id.includes("firstDimV")) {
          ...
          currentObj = await getObjectById(canvas, "mainImg" + textObj.id.replace("firstDimH", ""));
          distance = editedDim * scale - currentObj.width;;
