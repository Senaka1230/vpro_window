...
function Features({ className, ...rest }) {
  ...
  const handleCanvasMenuChange = async (index) => {
    ...
    canvas.clear();
    canvas.off();
    await mouseMoving(canvas, scale[index], createria[index], setRestoreNum, setCanvasState, setDeleteFlag);
  ...
  const handleCreateNewOne = async () => {
    ...
    if (leftMenu === 0) {
      ...
      await handleCreate(canvas, setScale, setCreateria, setCanvasState, setRestoreNum, setDeleteFlag, Number(width), Number(height), 0);
      ...
      await handleCreate(canvas, setScale, setCreateria, setCanvasState, setRestoreNum, setDeleteFlag, Number(width), Number(height), 1);
      const beforeState = await canvas.toJSON(['id', 'width', 'height', 'left', 'top', 'stroke', 'strokeWidth', 'selectable', 'hasControls', 'zIndex']);
      setCanvasMenu((prevCanvasState) => [...prevCanvasState, beforeState]);
      setRightMenuImg((prevCanvasState) => [...prevCanvasState, canvas.toDataURL({ format: 'png', quality: 1, })]);
      setRightMenuIndex(canvasMenu.length);
      ...
      await handleCreate(canvas, setScale, setCreateria, setCanvasState, setRestoreNum, setDeleteFlag, Number(width), Number(height), 2);
      const beforeState = await canvas.toJSON(['id', 'width', 'height', 'left', 'top', 'stroke', 'strokeWidth', 'selectable', 'hasControls', 'zIndex']);
      setCanvasMenu((prevCanvasState) => [...prevCanvasState, beforeState]);
      setRightMenuImg((prevCanvasState) => [...prevCanvasState, canvas.toDataURL({ format: 'png', quality: 1, })]);
      setRightMenuIndex(canvasMenu.length);
      ...
      await handleCreate(canvas, setScale, setCreateria, setCanvasState, setRestoreNum, setDeleteFlag, Number(width), Number(height), 3);
      const beforeState = await canvas.toJSON(['id', 'width', 'height', 'left', 'top', 'stroke', 'strokeWidth', 'selectable', 'hasControls', 'zIndex']);
      setCanvasMenu((prevCanvasState) => [...prevCanvasState, beforeState]);
      setRightMenuImg((prevCanvasState) => [...prevCanvasState, canvas.toDataURL({ format: 'png', quality: 1, })]);
      setRightMenuIndex(canvasMenu.length);
      ...
      await handleCreate(canvas, setScale, setCreateria, setCanvasState, setRestoreNum, setDeleteFlag, Number(width), Number(height), 4);
      const beforeState = await canvas.toJSON(['id', 'width', 'height', 'left', 'top', 'stroke', 'strokeWidth', 'selectable', 'hasControls', 'zIndex']);
      setCanvasMenu((prevCanvasState) => [...prevCanvasState, beforeState]);
      setRightMenuImg((prevCanvasState) => [...prevCanvasState, canvas.toDataURL({ format: 'png', quality: 1, })]);
      setRightMenuIndex(canvasMenu.length);
      ...
      await handleCreate(canvas, setScale, setCreateria, setCanvasState, setRestoreNum, setDeleteFlag, Number(width), Number(height), 5);
      const beforeState = await canvas.toJSON(['id', 'width', 'height', 'left', 'top', 'stroke', 'strokeWidth', 'selectable', 'hasControls', 'zIndex']);
      setCanvasMenu((prevCanvasState) => [...prevCanvasState, beforeState]);
      setRightMenuImg((prevCanvasState) => [...prevCanvasState, canvas.toDataURL({ format: 'png', quality: 1, })]);
      setRightMenuIndex(canvasMenu.length);
  ...
  const handleRightMenuDelete = async (index) => {
    ...
    if (!cashFlag)
      return;
    const newArray = [...canvasMenu];
    newArray.splice(index, 1);
    const newArray1 = [...rightMenuImg];
    newArray1.splice(index, 1);
    const newArray2 = [...scale];
    newArray2.splice(index, 1);
    const newArray3 = [...createria];
  ...
  const handleRightMenuDuplicate = async (index) => {
    const cashArray = [...canvasMenu];
    const copiedElement = cashArray[index];
    const cashArray1 = [...rightMenuImg];
    const copiedElement1 = cashArray1[index];
    const cashArray2 = [...scale];
    const copiedElement2 = cashArray2[index];
    const cashArray3 = [...createria];
    const copiedElement3 = cashArray3[index];
    var newArray, newArray1, newArray2, newArray3;
          ...
          <Box display='flex'>
            <Box display='flex' justifyContent='center' flexDirection='column' style={{ marginLeft: "15px", marginTop: "5px" }}>
              <img alt='rect' src='/static/images/EditWindowVertical.png' width="30px" height='auto' style={{ marginRight: "5px" }} className={classes.hover}
                onClick={() => {
                  handleVerticalSplitWindows(canvas, rightMenuIndex, scale[rightMenuIndex], createria[rightMenuIndex], setCreateria, "T", setCanvasState, setRestoreNum, setDeleteFlag);
                }}
              />
              <img alt='rect' src='/static/images/EditWindowHorizontal.png' width="30px" height='auto' className={classes.hover}
                onClick={() => { handleHorizontalSplitWindows(canvas, rightMenuIndex, scale[rightMenuIndex], createria[rightMenuIndex], setCreateria, "L", setCanvasState, setRestoreNum, setDeleteFlag) }}
              />
            </Box>
            <Box display='flex' justifyContent='center' flexDirection='column' style={{ marginTop: "5px" }}>
              <img alt='rect' src='/static/images/EditWindowVertical.png' width="30px" height='auto' style={{ marginRight: "5px", transform: "scaleX(-1) rotate(180deg)" }} className={classes.hover}
                onClick={() => { handleVerticalSplitWindows(canvas, rightMenuIndex, scale[rightMenuIndex], createria[rightMenuIndex], setCreateria, "B", setCanvasState, setRestoreNum, setDeleteFlag) }}
              />
              <img alt='rect' src='/static/images/EditWindowHorizontal.png' width="30px" height='auto' style={{ transform: "scaleX(-1)" }} className={classes.hover}
                onClick={() => { handleHorizontalSplitWindows(canvas, rightMenuIndex, scale[rightMenuIndex], createria[rightMenuIndex], setCreateria, "R", setCanvasState, setRestoreNum, setDeleteFlag) }}
              />
            </Box>
          </Box>
      ...
      <Box display='flex' justifyContent='right'>
        <Box style={{ margin: "10px", paddingRight:"25px", overflowY: 'auto', overflowX: 'hidden', maxHeight: window.innerHeight - 250}}>
          {rightMenuImg.map((item, index) => (
            <Box key={index} style={{ marginLeft: "25px", marginTop: (index === 0) ? "20px" : "-110px" }} onClick={() => { handleCanvasMenuChange(index) }}>
              <img src={item} alt="Canvas" width="350px" height="200px" style={{ border: (index !== rightMenuIndex) ? "solid 1px black" : "solid 2px blue" }} />
